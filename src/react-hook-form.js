...
var getEventValue = (event) =>
  isObject(event) && event.target ? (isCheckBoxInput(event.target) ? event.target.checked : event.target.value) : event
...
var shouldSubscribeByName = (name, signalName, exact) =>
  ...
  convertToArrayPayload(name).some(
    (currentName) =>
      currentName &&
      (exact ? currentName === signalName : currentName.startsWith(signalName) || signalName.startsWith(currentName)),
  )
...
function Form(props) {
    ...
    await control.handleSubmit(async (data) => {
      ...
      if (action) {
        try {
          ...
          if (
            response &&
            (validateStatus ? !validateStatus(response.status) : response.status < 200 || response.status >= 300)
          ) {
            hasError = true
  ...
  return render
    ...
    : React.createElement(
        'form',
        { noValidate: mounted, action: action, method: method, encType: encType, onSubmit: submit, ...rest },
        children,
      )
}
...
var getFocusFieldName = (name, index, options = {}) =>
  options.shouldFocus || isUndefined(options.shouldFocus)
    ? options.focusName || `${name}.${isUndefined(options.focusIndex) ? index : options.focusIndex}.`
    : ''
...
function unset(object, path) {
  ...
  if (
    index !== 0 &&
    ((isObject(childObject) && isEmptyObject(childObject)) || (Array.isArray(childObject) && isEmptyArray(childObject)))
  ) {
...
function createFormControl(props = {}) {
  ...
  const updateTouchAndDirty = (name, fieldValue, isBlurEvent, shouldDirty, shouldRender) => {
    ...
    if (!isBlurEvent || shouldDirty) {
      ...
      shouldUpdateField =
        shouldUpdateField || (_proxyFormState.dirtyFields && isPreviousDirty !== !isCurrentFieldPristine)
    }
    if (isBlurEvent) {
      ...
      if (!isPreviousFieldTouched) {
        ...
        shouldUpdateField =
          shouldUpdateField || (_proxyFormState.touchedFields && isPreviousFieldTouched !== isBlurEvent)
      }
    }
  ...
  const _executeSchema = async (name) => {
    ...
    const result = await _options.resolver(
      _formValues,
      _options.context,
      getResolverOptions(name || _names.mount, _fields, _options.criteriaMode, _options.shouldUseNativeValidation),
    )
  ...
  const setValues = (name, value, options) => {
    for (const fieldKey in value) {
      ...
      const field = get(_fields, fieldName)
      ;(_names.array.has(name) || !isPrimitive(fieldValue) || (field && !field._f)) && !isDateObject(fieldValue)
  ...
  const onChange = async (event) => {
    ...
    if (field) {
      ...
      if (_options.resolver) {
        ...
        _updateIsValidating([name], true)
        error = (
          await validateField(field, _formValues, shouldDisplayAllAssociatedErrors, _options.shouldUseNativeValidation)
        )[name]
        _updateIsValidating([name])
        _updateIsFieldValueUpdated(fieldValue)
        if (isFieldValueUpdated) {
