...
> {
  ...
  injectInto<NewReducerPath extends string = ReducerPath>(
    this: this,
    injectable: {
      inject: (slice: { reducerPath: string; reducer: Reducer }, config?: InjectConfig) => void
    },
...
interface InjectedSlice<
  ...
  ReducerPath extends string = Name,
  Selectors extends SliceSelectors<State> = SliceSelectors<State>,
> extends Omit<Slice<State, CaseReducers, Name, ReducerPath, Selectors>, 'getSelectors' | 'selectors'> {
...
export interface CaseReducerWithPrepareDefinition<State, Action extends PayloadAction>
...
export interface AsyncThunkSliceReducerDefinition<
  ...
  Returned = unknown,
  ThunkApiConfig extends AsyncThunkConfig = {},
> extends AsyncThunkSliceReducerConfig<State, ThunkArg, Returned, ThunkApiConfig>,
...
> {
  <Returned, ThunkArg = void>(
    ...
    config?: AsyncThunkSliceReducerConfig<State, ThunkArg, Returned, CurriedThunkApiConfig>,
  ): AsyncThunkSliceReducerDefinition<State, ThunkArg, Returned, CurriedThunkApiConfig>

  <Returned, ThunkArg, ThunkApiConfig extends PreventCircular<AsyncThunkConfig> = {}>(
    payloadCreator: AsyncThunkPayloadCreator<Returned, ThunkArg, ThunkApiConfig>,
    config?: AsyncThunkSliceReducerConfig<State, ThunkArg, Returned, ThunkApiConfig>,
  ): AsyncThunkSliceReducerDefinition<State, ThunkArg, Returned, ThunkApiConfig>
  withTypes<ThunkApiConfig extends PreventCircular<AsyncThunkConfig>>(): AsyncThunkCreator<
    State,
...
type SliceActionType<SliceName extends string, ActionName extends keyof any> = ActionName extends string | number
...
type ActionCreatorForCaseReducer<CR, Type extends string> = CR extends (state: any, action: infer Action) => any
...
type SliceDefinedCaseReducers<CaseReducers extends SliceCaseReducers<any>> = {
  [Type in keyof CaseReducers]: CaseReducers[Type] extends infer Definition
    ? Definition extends AsyncThunkSliceReducerDefinition<any, any, any, any>
      ? Id<Pick<Required<Definition>, 'fulfilled'>>
      : Definition extends {
...
type RemappedSelector<S extends Selector, NewState> = S extends Selector<any, infer R, infer P>
...
export function buildCreateSlice({ creators }: BuildCreateSliceConfig = {}) {
  ...
  >(
    ...
    const reducers =
      (typeof options.reducers === 'function' ? options.reducers(buildReducerCreators<State>()) : options.reducers) ||
      {}
    ...
    const injectedSelectorCache = new Map<
      boolean,
      WeakMap<(rootState: any) => State | undefined, Record<string, (rootState: any) => any>>
    >()
...
interface ReducerHandlingContext<State> {
  sliceCaseReducersByName: Record<
    string,
    CaseReducer<State, any> | Pick<AsyncThunkSliceReducerDefinition<State, any, any, any>, 'fulfilled'>
  >
...
interface ReducerHandlingContextMethods<State> {
  ...
  exposeCaseReducer(
    name: string,
    reducer: CaseReducer<State, any> | Pick<AsyncThunkSliceReducerDefinition<State, any, any, any>, 'fulfilled'>,
  ): ReducerHandlingContextMethods<State>
}
...
function handleThunkCaseReducerDefinition<State>(
  ...
  const { payloadCreator, fulfilled, options } = reducerDefinition
  const thunk = cAT(type, payloadCreator)
